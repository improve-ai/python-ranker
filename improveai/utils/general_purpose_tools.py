...
def check_items(items: list or tuple or np.ndarray) -> list or tuple or np.ndarray:
    """
    ...
    assert type(items) in ALLOWED_VARIANT_COLLECTION_TYPES
    # raise if variants are an empty list
    assert len(items) > 0, '`variants` must not be an empty collection'
