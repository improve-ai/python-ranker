...
get_noise_shift_scale = cfe.get_noise_shift_scale
scale = cfe.scale
get_mask = cfe.get_mask
...
class TestEncoder(TestCase):
    ...
    def test_encode_raises_for_feature_vector_item_nan_context_none_into_none(self):
        ...
        with raises(ValueError) as verr:
            ...
            assert verr.value is not None

        np.testing.assert_array_equal(test_into, np.array([None, None]))

    def test_encode_raises_for_feature_vector_item_None_context_none_into_nan(self):
    ...
    def test_noise_out_of_bounds_raises(self):
        ...
        with raises(AssertionError) as aerr:
            get_noise_shift_scale(99)
            assert str(aerr.value)

        with raises(AssertionError) as aerr:
            get_noise_shift_scale(-1.0)
            assert str(aerr.value)
    ...
    def test_encode_item_raises_for_non_json_encodable(self):
        ...
        with raises(ValueError) as verr:
            feature_encoder.encode_item(object, np.array([np.nan, np.nan]))

        with raises(ValueError) as verr:
            feature_encoder.encode_item(np.array([1, 2, 3]), np.array([np.nan, np.nan]))

    def test_encode_context_raises_for_non_json_encodable(self):
        feature_encoder = FeatureEncoder(
            feature_names=['item', 'context'], string_tables={}, model_seed=1)
        with raises(ValueError) as verr:
            feature_encoder.encode_context(object, np.array([np.nan, np.nan]))

        with raises(ValueError) as verr:
            feature_encoder.encode_context(np.array([1, 2, 3]), np.array([np.nan, np.nan]))
